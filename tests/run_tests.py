...
class TestExecuteOrdersToOpen(unittest.TestCase):
  ...
  def test_margin_scales_with_quantity(self):
    ...
    order = Order('s&p500', 'buy', 5, Decimal('90'), Decimal('110'), self.day1)
    trader.submit_order(order)
    broker.execute_orders_to_open(self.day1)
...
class TestPnlScalesWithQuantity(unittest.TestCase):
  ...
  def open_position_qty5(self, datafeed):
    (orderQ, receiptQ, term_req_Q, term_notice_Q, broker, trader) = \
      make_broker_and_trader(datafeed, Decimal('10000'), 's&p500', self.day1, self.day2)
    order = Order('s&p500', 'buy', 5, Decimal('90'), Decimal('110'), self.day1)
    trader.submit_order(order)
    broker.execute_orders_to_open(self.day1)
  ...
  def test_take_profit_scales_with_quantity(self):
    datafeed = FakeDataFeed({
      self.day1: {'high': Decimal('105'), 'low': Decimal('95'), 'close': Decimal('100')},
      self.day2: {'high': Decimal('115'), 'low': Decimal('108'), 'close': Decimal('112')},
    })
    broker, trader = self.open_position_qty5(datafeed)

    broker.manage_open_positions(self.day2)
  ...
  def test_stop_loss_forfeits_scaled_margin(self):
    datafeed = FakeDataFeed({
      self.day1: {'high': Decimal('105'), 'low': Decimal('95'), 'close': Decimal('100')},
      self.day2: {'high': Decimal('92'), 'low': Decimal('85'), 'close': Decimal('88')},
    })
    broker, trader = self.open_position_qty5(datafeed)
  ...
  def test_close_at_price_scales_with_quantity(self):
    ...
    broker.execute_orders_to_close(self.day2)
